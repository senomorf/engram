...
- A screen test that mutates the db mid-composition (QueueScreen's reconcile) must close the db only after the composition disposes: put the compose rule at `@get:Rule(order = 1)` inside an outer `ExternalResource` (`order = 0`) that closes it, else v2's queued queue() re-query races app.db.close() (SQLiteConnectionPool ISE, the race EngramDb.inMemory's inline executor prevents under v1).
