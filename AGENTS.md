...
- coverage gates + reports: `koverVerify` (per-module floors) runs inside `check`, `./gradlew koverVerifyAggregate` enforces the combined 96% (both in CI); HTML via `:core-format:koverHtmlReport`, `:cli:koverHtmlReport`, `:app:koverHtmlReportDebug`
...
- Release gate (Track A v1 exit criteria, docs/plan.md): do not promote a non-rc tag until the survivability-matrix rows 1 to 11 and the landmine verdicts are recorded in lab/survivability-matrix.md and the docs/device-qa.md pass is done. Now CI-enforced (finding F7): release.yml's `verify` job gates signing (`release` needs it) on the full build/tests/coverage for the tag SHA, on the tag being reachable from origin/main (ancestry: no releasing an unmerged commit), and on `scripts/check-release-evidence.sh` (unit-tested in `scripts/check-release-evidence.test.sh`) finding the matrix filled and the device-qa boxes checked. rc tags (hyphen suffix) skip the ancestry + evidence checks so the pipeline can still be trialed on any commit.
...
- Room suspend DAOs run off the test scheduler (never advanceUntilIdle): ViewModel tests drive real coroutines with runBlocking; prefer awaiting the resulting StateFlow via `awaitValue { predicate }` (ScreenHarness) over a fixed delay() settle, so a test resolves as soon as the write lands (a few ViewModel tests still use the older delay() and are being migrated).
- Screen tests use the v2 `createComposeRule` (`androidx.compose.ui.test.junit4.v2`): its StandardTestDispatcher (queued, virtual clock) makes the LaunchedEffect->ViewModel->StateFlow->recompose async deterministic and order-independent, so no `Dispatchers.setMain(Unconfined)` is needed (the rule idles Robolectric's main looper) and the recovery-banner check lives in QueueScreenTest beside its siblings (the old order-dependent split is gone, issue #99). v1 `createComposeRule` is deprecated; don't reintroduce it. setScreen still gives each composition its own ViewModelStore cleared on teardown (cancels the screen's viewModelScope / stateIn WhileSubscribed flows); keep it.
- Screen tests extend `ScreenTest`: its `@get:Rule(order = 1)` compose rule disposes the composition (cancelling each screen's viewModelScope / Room-flow subscription) before its outer `@get:Rule(order = 0)` ExternalResource closes the containers registered with `fakeContainer().closingDb()`. Don't hand-roll `app.db.close()` in `@After` for a screen test, and don't reintroduce the old inline two-rule pattern: without this order a screen mutating an observed table while subscribed (QueueScreen's reconcile) races the v2 queued re-query against a closed pool (SQLiteConnectionPool ISE; EngramDb.inMemory's inline executor prevents it under v1).
...
- Records are append-only; nothing may silently drop or rewrite existing payload. The IPTC caption upsert likewise preserves other IIM datasets (keywords, by-line, copyright), extended-length datasets included, byte-exact; only 2:120 is replaced, and an IIM that cannot be safely parsed skips the caption mirror (leaves APP13 untouched) rather than dropping datasets. An ExtendedXMP split copies every dc:description language into the standard packet (XmpCoreEngine.copyDescriptions sets the specific langs first, then x-default via setArrayItem to dodge Adobe setLocalizedText's x-default sync), so an oversized foreign multi-language caption is not dropped on split.
...
- Media access is FULL/PARTIAL/DENIED (MediaPermissions.state; READ_MEDIA_VISUAL_USER_SELECTED declared). The reconcile prune runs only under FULL access (Reconciler.hasFullMediaAccess): never drop that guard, or a lapsed/partial Android 14 grant returns a subset snapshot that wipes the index (design.md D31, finding H5). On a reused id (a known id whose takenAtMillis changed), the identity-driven media-row replacement and its id-keyed enrichment/draft eviction commit in one db.withTransaction: never split them, or a crash between them strands the reused id's old private draft on the new photo (design.md D29, findings H1/F3). The prune also evicts each removed id's draft in that same transaction (drafts have no cascade FK), so a later reused id arriving through the known == null new-item path cannot inherit the old capture's private draft.
- Content writes are a WriteResult tri-state (NotOpened/OpenedUncertain/Ok), not Boolean; durable .meta-before-.bak backup, the .bak is deleted only after the new file verifies intact (every expected record id present) or the original is restored, a new attempt resolves any pending journal first and bails without touching it when it cannot, a Mutex serializes write and recovery (design.md D26). Output preparation (embed, MP4 temp) runs before the target is opened: a preparation failure cleans the journal and never restores; a journal whose target digest equals its backup resolves grant-free (WriteJournal). WriteJournal.resolve returns a typed Resolution (Settled/NeedsConsent/Unresolved), not Boolean: a restore that cannot open the target (grant lost after process death) surfaces NeedsConsent, recoverPending returns those uris, and EngramRoot (launch prompt) plus the Queue affordance re-request consent via the createWriteRequest gate; the foreground path maps NeedsConsent to NotOpened so the same consent UI fires (finding C2). A reused MediaStore id is caught by capture identity: writeSidecar persists takenAtMillis as the 5th .meta line, and resolve orphans the .bak (renamed out of pendingBackups, never deleted) on a positive DATE_TAKEN mismatch rather than overwrite the unrelated new photo (finding F1). Verify and writeCompleted also require structural completeness (ScanOutcome.structurallyComplete via PngCodec.isComplete: first IHDR, last IEND) before deleting the .bak, so a PNG truncated before IEND is rolled back or restored, never blessed; ResolverContentAccess.writeBytes/writeFromFile fsync so a page-cache tail cannot pass verify then truncate (finding F2). Record ids are content-addressed (RecordFactory hashes mediaId, the draft's createdAtMillis, kind, payload; never re-random them, and never drop the draft stamp or re-saving earlier text collides with the record already present) and writeRecords appends only ids RecordScanner.presentIds does not already report: a save retried after a crash between the committed write and the draft delete re-derives the same ids and no-ops, so the append-only file never gains duplicate note/audio records.
...
- Coverage (D22): per-module `koverVerify` floors (core-format/cli 97, app 93) run in `check`; root `koverVerifyAggregate` defends the combined 96% (both enforced in CI). Floors only rise, kept below achieved ~97.5/97.7/94.6 (aggregate ~96.3) to absorb ~0.5% Compose-timing variance. Counted coverage is JVM/Robolectric only (Kover cannot measure on-device); real platform adapters (MediaStore, SAF, SpeechRecognizer, MediaRecorder, Geocoder), LabScreen, and device-only Compose marked `@DeviceOnly` are excluded and covered by the instrumented androidTest layer.
