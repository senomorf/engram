...
- Screen tests: setScreen (ScreenHarness) gives each composition its own ViewModelStore and clears it on teardown, cancelling the screen's ViewModel (viewModelScope coroutines, stateIn WhileSubscribed flows). Without it those flows leak across tests and starve a later Compose test's recompose, so screen tests (e.g. QueueScreen) flake in the full suite while passing in isolation. Keep it.
