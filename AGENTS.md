...
- Content writes are a WriteResult tri-state (NotOpened/OpenedUncertain/Ok), not Boolean; durable .meta-before-.bak backup, the .bak is deleted only after the new file verifies intact (every expected record id present) or the original is restored, a new attempt resolves any pending journal first and bails without touching it when it cannot, a Mutex serializes write and recovery (design.md D26). Output preparation (embed, MP4 temp) runs before the target is opened: a preparation failure cleans the journal and never restores; a journal whose target digest equals its backup resolves grant-free (WriteJournal).
